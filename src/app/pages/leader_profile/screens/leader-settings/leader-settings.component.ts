...
import { confirmPasswordValidator } from '../../../../shared/services/confirmPasswordValidator';
...
export class LeaderSettingsComponent implements OnInit {
  ...
  getGeneralProfile(): void {
    this.isLoading = true;
    this.leaderProfileService.accountInfo().subscribe({
  ...
  vaildationEmail(): void {
    ...
    if (email) {
      this.leaderProfileService.validateEmail(email).subscribe({
  ...
  vaildationUsername(): void {
    ...
    if (username) {
      this.leaderProfileService.validateUsername(username).subscribe({
        next: ({ statusCode, message }) => {
          if (statusCode === 200) {
  ...
  vaildationCodeforces(): void {
    ...
    if (codeforces) {
      this.leaderProfileService.validateCodeForcesHandle(codeforces).subscribe({
        next: ({ statusCode, message }) => {
          if (statusCode === 200) {
  ...
  vaildationVjudge(): void {
    ...
    if (vjudge) {
      this.leaderProfileService.validateVjudgeHandle(vjudge).subscribe({
        next: ({ statusCode, message }) => {
          if (statusCode === 200) {
